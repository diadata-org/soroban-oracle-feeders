...
export async function updateOracle(keys: string[], prices: number[]) {
  ...
  const wallet =  new PrivateKeyWallet({privateKey: config.alephium.secretKey, keyType: undefined, nodeProvider: nodeProvider})
  ...
  for (const ketBatchIndex in keyBatches) {
    ...
    const result = await SetMultipleValues.execute(wallet, {
      initialFields: {
        keys: fillArray(keyBatch.map(key => stringToHex(key)), config.alephium.maxBatchSize, stringToHex('')),
        values: fillArray(priceBatch.map(price => BigInt(Math.floor(price * 100_000_000))), config.alephium.maxBatchSize, 0n),
